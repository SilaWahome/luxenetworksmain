...
if ($action == 'verify_phone') {
    ...
    // 2. Not confirmed yet, check Google Sheets registration
    $data = fetchRegistrations();

    if (!$data || !isset($data['data'])) {
        $error_msg = 'Could not fetch registration list.';
        if (isset($data['error'])) {
    ...
    foreach ($data['data'] as $entry) {
        $regPhone = normalizePhone($entry['Drop your whatsapp phone number Kindly'] ?? '');
    ...
    if ($found) {
        echo json_encode([
            'success' => true, 
            'name' => $found['What are the names of people attending?'],
            'phone' => $normalized
        ]);
    } else {
        echo json_encode(['success' => false, 'message' => 'Phone number not found in registration list.']);
    }
} 
