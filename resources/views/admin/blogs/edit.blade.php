...
<div class="flex items-center md:justify-between flex-wrap gap-2 mb-6">
    <h4 class="text-default-900 text-lg font-semibold">Edit Blog</h4>
    ...
    <div class="md:flex hidden items-center gap-3 text-sm font-semibold">
        ...
        <i class="ti ti-chevron-right text-lg flex-shrink-0 text-default-500 rtl:rotate-180"></i>
        <a href="#" class="text-sm font-medium text-default-700" aria-current="page">Edit</a>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title', $blog->title) }}" placeholder="Enter the blog title..." class="form-input text-sm w-full border border-gray-300 rounded p-2">
            </div>

            <div>
                <label class="text-xs font-bold uppercase text-default-500 mb-1 block">Cover Image (Leave empty to keep current)</label>
                @if($blog->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $blog->image) }}" alt="Current Image" class="h-20 rounded">
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea id="blog-content" name="content" class="form-input w-full">{{ old('content', $blog->content) }}</textarea>
            </div>
            ...
            <div class="pt-4 border-t border-default-200">
                <button type="submit" class="btn bg-primary text-white py-2 px-6 rounded-md font-bold text-sm uppercase">Update Blog</button>
